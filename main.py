...
if ALPHA_VANTAGE_API_KEY is None:
    print("ERROR: Alpha Vantage API key not found. Make sure it's in your .env file!")
    exit() # Stop the program if the key isn't found
...
with st.expander('-- Click here to expand --'):
    ...
    def load_data(ticker):
        ...
        """
        data, meta_data = ts_av.get_daily_adjusted(ticker, outputsize='full')
    ...
    data.columns = ['Date', 'Open', 'High', 'Low', 'Close', 'Adjusted Close', 'Volume', 'Dividend Amount', 'Split Coefficient']
    data = data[::-1]
...
indicator_bb = ta.volatility.BollingerBands(close=data[price_col], window=20, window_dev=2)
...
data = dynamic_winsorize(data, price_col)

# Get training data
# ------------------------------------------------------------------
df_train = data[['Date', price_col, 'winsorized']]
...
def model_drafts(df_train, scores_df=scores_df):
    """
    Trains Prophet models on 'Adjusted Close' and 'winsorized' columns, evaluates their performance, and updates the scores DataFrame.

    Parameters:
    df_train (pd.DataFrame): DataFrame containing the training data with 'ds' (date) and 'Adjusted Close'/'winsorized' columns.
    scores_df (pd.DataFrame): DataFrame to store the performance metrics of the models.

    Returns:
    pd.DataFrame: Updated scores DataFrame with performance metrics for 'Adjusted Close' and 'winsorized' models.
    """
    for i in [price_col, 'winsorized']:
        m = Prophet()
...
forecast_df = pd.merge(
    ...
    right_on='ds',
    left_on='Date',
    how='right')[['ds', 'Adjusted Close', 'yhat', 'yhat_lower', 'yhat_upper', 'SMA50', 'bb_upper', 'bb_lower']]
forecast_df.rename(columns={'ds': 'Date'}, inplace=True) # keep naming convention and ds data. Date does not contain forecast date values.
...
def plot_forecast(data):
    """
    Generates a Plotly figure for the forecasted line graph.

    Parameters:
    data (pd.DataFrame): DataFrame containing the forecast data and indicators.

    Returns:
    go.Figure: Plotly figure object with the forecasted line graph.
    """
    fig = go.Figure()
    ...
    fig.add_trace(go.Scatter(x=data['Date'], 
                            ...
                            visible='legendonly'))
    # Add line graph for Adjusted Close price
