    ...
    if not forecast_df.empty:
        ...
        if len(forecast_only_df) > 0:
            ...
            if target_day == preferred_target_day:
                st.subheader(f"{target_day}-Trading-Day Forecast Summary for {selected_stock}")
            else:
                st.subheader(f"{target_day}-Trading-Day Forecast Summary for {selected_stock}")
                st.info(f"Note: Showing {target_day} trading days forecast (maximum available with current settings). Increase training duration for longer forecasts.")
            
            # Get the forecast row for the target day
            forecast_row = forecast_only_df.iloc[target_day - 1]  # -1 because index starts at 0
