    ...
    with st.expander("Click to view detailed BigQuery diagnostics", expanded=False):
        ...
        try:
            ...
            if bq_client:
                ...
                with col1:
                    status_icon = (
                        "✅" if detailed_status["connection_available"] else "❌"
                    )
                    st.metric(
                        "Connection Status",
                        (
                            "Available"
                            if detailed_status["connection_available"]
                            else "Failed"
                        ),
                        delta=None,
                    )
                ...
                if test_results:
                    ...
                    for step_key, step_name in test_steps:
                        if step_key in test_results:
                            ...
                            st.write(
                                f"{status_icon} **{step_name}**: {'PASS' if result else 'FAIL'}"
                            )
                ...
                env_info = {
                    "Streamlit Available": detailed_status.get(
                        "streamlit_available", False
                    ),
                    "Streamlit Secrets Available": detailed_status.get(
                        "streamlit_secrets_available", False
                    ),
                    "Environment Variable Available": detailed_status.get(
                        "env_var_available", False
                    ),
                    "Client Exists": detailed_status.get("client_exists", False),
                    "Credentials Exist": detailed_status.get(
    ...
    try:
        ...
        if bq_client and bq_client.is_available():
            ...
            if bq_client:
                ...
                if failed_steps:
                    step_name = (
                        failed_steps[0].replace("step_", "").replace("_", " ").title()
                    )
